...
func newTestRegistry(t *testing.T, provider *agenttest.MockProvider, opts ...agent.RegistryOption) *agent.ProviderRegistry {
	t.Helper()
	pidDir := t.TempDir()
	reg, err := agent.NewProviderRegistry(provider, pidDir, opts...)
	if err != nil {
...
func TestGetOrSpawn_NewInstance(t *testing.T) {
	...
	adapter, err := reg.GetOrSpawn(context.Background(), "/worktree/a")
	if err != nil {
	...
	if inst := reg.GetInstance("/worktree/a"); inst == nil {
...
func TestGetOrSpawn_SameWorktreeReturnsExisting(t *testing.T) {
	...
	a1, _ := reg.GetOrSpawn(context.Background(), "/worktree/a")
	a2, _ := reg.GetOrSpawn(context.Background(), "/worktree/a")
...
func TestGetOrSpawn_DifferentWorktreeSpawnsNew(t *testing.T) {
	...
	a1, _ := reg.GetOrSpawn(context.Background(), "/worktree/a")
	a2, _ := reg.GetOrSpawn(context.Background(), "/worktree/b")
...
func TestGetOrSpawn_SpawnFailure(t *testing.T) {
	...
	_, err := reg.GetOrSpawn(context.Background(), "/worktree/a")
	if err == nil {
...
func TestEviction_LRUIdle(t *testing.T) {
	...
	// Spawn A and B.
	reg.GetOrSpawn(context.Background(), "/worktree/a")
	time.Sleep(10 * time.Millisecond)
	reg.GetOrSpawn(context.Background(), "/worktree/b")

	// Touch A so B is the LRU.
	time.Sleep(10 * time.Millisecond)
	reg.TouchActivity("/worktree/a")

	// Spawn C — should evict B.
	_, err := reg.GetOrSpawn(context.Background(), "/worktree/c")
	if err != nil {
		t.Fatalf("GetOrSpawn /c: %v", err)
	}

	if reg.GetInstance("/worktree/b") != nil {
		t.Error("expected /worktree/b to be evicted")
	}
	if reg.GetInstance("/worktree/a") == nil {
		t.Error("expected /worktree/a to survive")
	}
	if reg.GetInstance("/worktree/c") == nil {
...
func TestEviction_AllBusy(t *testing.T) {
	...
	// Mark both busy.
	reg.MarkBusy("/worktree/a")
	reg.MarkBusy("/worktree/b")
...
func TestActivityTracking(t *testing.T) {
	...
	reg.GetOrSpawn(context.Background(), "/worktree/a")
	inst := reg.GetInstance("/worktree/a")
...
func TestStopInstance(t *testing.T) {
	...
	reg.GetOrSpawn(context.Background(), "/worktree/a")
	if err := reg.StopInstance("/worktree/a"); err != nil {
...
func TestStopInstance_NotFound(t *testing.T) {
	...
	if err := reg.StopInstance("/worktree/nonexistent"); err == nil {
...
func TestSweepOrphans_KillsLiveProcess(t *testing.T) {
	...
	p := agenttest.NewMockProvider()
	reg, err := agent.NewProviderRegistry(p, pidDir)
	if err != nil {
		t.Fatalf("NewProviderRegistry: %v", err)
	}
	defer reg.Close()
...
func TestSSEAggregation(t *testing.T) {
	...
	instA := reg.GetInstance("/worktree/a")
	instB := reg.GetInstance("/worktree/b")
...
func TestGetOrSpawn_ConcurrentSameWorktree(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		go func(idx int) {
			defer wg.Done()
			a, err := reg.GetOrSpawn(context.Background(), "/worktree/a")
			if err != nil {
